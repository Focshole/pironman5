...
class ConfigTxt(object):
    ...
    def comment(self, expected):
        for i in range(len(self.configs)):
            if expected in self.configs[i] and not self.configs[i].startswith("#"):
                self.configs[i] = "#" + self.configs[i]
        return self.write_file()
...
class SF_Installer:
    WORK_DIR = "/opt/{name}"
    APT_DEPENDENCIES = ["python3-pip", "python3-venv", "git"]
    PACMAN_DEPENDENCIES = ["python3-pip", "python3-venv", "git"]
    PIP_DEPENDENCIES = ["pip", "setuptools", "build"]
    ...
    def __init__(
        ...
        self.parser.add_argument(
            "--no-dep", action="store_true", help="Do not install dependencies"
        )
        self.parser.add_argument(
            "--skip-reboot", action="store_true", help="Skip reboot even needed"
        )
        self.parser.add_argument(
            "--plain-text", action="store_true", help="Plain text mode"
        )
        self.parser.add_argument(
            "--skip-auto-start", action="store_true", help="Skip auto start"
        )
        self.parser.add_argument(
            "--skip-config-txt", action="store_true", help="Skip config.txt"
        )
        self.parser.add_argument(
            "--skip-dtoverlay", action="store_true", help="Skip dtoverlay"
        )
        self.parser.add_argument(
            "--skip-modules", action="store_true", help="Skip probe modules"
        )
    ...
    def spinner(self):
        ...
        while self.is_running:
            i = (i + 1) % 4
            sys.stdout.write("\033[?25l" + char[i] + "\033[1D")
            sys.stdout.flush()
            time.sleep(0.5)
    ...
    def install_python_source(self, name, url="./"):
        self.do(f"Uninstall {name} old package", f"{self.venv_pip} uninstall -y {name}")
        self.do(f"Install {name} from source", f"{self.venv_pip} install {url}")
    ...
    def install_apt_dep(self):
        if self.args.no_dep or len(self.custom_apt_dependencies) == 0:
            return
    ...
    def create_working_dir(self):
        ...
        try:
            self.do(
                ...
                f'python3 -m venv {self.venv_path} {" ".join(self.venv_options)}',
            )
        except Exception as e:
    ...
    def install_py_src_pkgs(self):
        ...
        for package, url in self.python_source.items():
            ...
            self.do(f"Uninstall {package}", f"{self.venv_pip} uninstall -y {package}")
            self.do(f"Install {package} from source", f"{self.venv_pip} install {url}")
    ...
    def remove_dtoverlay(self):
        ...
        for overlay in self.dtoverlays:
            ...
            if not os.path.exists(full_path):
                print(f" - Overlay {overlay} not found. Skip.")
                continue
